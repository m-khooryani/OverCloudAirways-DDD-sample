...
public class FlightCapacityChangedPolicyTests
{
    [Fact]
    public async Task EnqueueProjectingReadModelFlightCapacityChangedPolicyHandler_Should_Enqueue_ProjectFlightReadModelCommand()
    {
        // Arrange
        var commandsScheduler = Substitute.For<ICommandsScheduler>();
        var handler = new EnqueueProjectingReadModelFlightCapacityChangedPolicyHandler(commandsScheduler);
        var policy = new FlightCapacityChangedPolicyBuilder().Build();
